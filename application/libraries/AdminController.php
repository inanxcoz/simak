...
class AdminController extends My_Controller{ 
	...
	var $enable_download = FALSE;
	var $enable_option_limit = TRUE;
	...
	function __construct(){  
		...
		if($this->_getClass()!= 'me' && $this->_getClass()!= 'map' ){
		...
		$this->arr_perpage = array(5,10,15,20,25,30,50,100,200);
		$this->load->helper('app');
		$this->last_activity();

		$this->per_page = isset($this->jCfg['search']['per_page'])?$this->jCfg['search']['per_page']:20;
	}
	
	function last_activity(){
		$this->db->update("app_user",array(
				'user_last_activity' => date("Y-m-d H:i:s"),
				'user_last_module'	 => $this->_getClass()."/".$this->_getMethod()
			),array(
