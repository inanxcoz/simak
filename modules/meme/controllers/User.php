...
class User extends AdminController {  
	...
	function export_data(){
        ...
        $data = $this->M->data_user($par_filter);
	...
	function save(){
		...
		if( trim($this->input->post('user_id'))=="" ){
			$user_name = str_replace(" ", "", trim($this->input->post('user_name')));
			$cek_user = $this->db->get_where("app_user",array(
					"user_name" => $user_name
				))->num_rows();
			if( $cek_user > 0 ){
				redirect($this->own_link."/add?msg=".urldecode('Username '.$user_name.' sudah digunakan')."&type_msg=danger");
			}
		}
		
		/*if($this->jCfg['user']['is_all'] == 1){
			$data['is_show_all'] = isset($_POST['is_show_all'])?1:0;
		}*/
