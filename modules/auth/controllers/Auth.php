...
class Auth extends FrontController {
		...
		if(isset($_POST['login'])){
			...
			if( trim($u) == '' || trim($p) == '' ){
				...
				if(count($d) > 0){					
					...
					$this->sCfg['user']['section']	= $d->user_sec_id;
					...
					$this->sCfg['user']['propinsi']	= $d->user_propinsi;
					$this->sCfg['user']['kabupaten']= $d->user_kabupaten;	
	...
	function out(){
		...
		$this->sCfg['user']['propinsi'] 	= 0;
		$this->sCfg['user']['kabupaten'] 	= 0;
