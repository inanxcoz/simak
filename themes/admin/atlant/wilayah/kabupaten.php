                ...
                <tbody style="text-align: center;">
                    ...
                    if (count($data) > 0) {
                        foreach ($data as $r) { ?>
                            <tr valign="top">
                                <td><?php echo ++$no; ?></td>
                                <td><?php echo strtoupper($r->kab_kode); ?></td>
                                <td><?php echo strtoupper($r->propinsi_nama); ?></td>
                                <td><?php echo strtoupper($r->kab_nama); ?></td>
                                <td><?php echo ($r->kab_status == 1) ? '<span class="label label-info">Aktif</span>' : '<span class="label label-warning">Tidak Aktif</span>'; ?></td>
                                <td align="right"><?php link_action($links_table_item, "?_id=" . _encrypt($r->kab_id)); ?></td>
                            </tr>
                    <?php }
                    } ?>
                </tbody>
