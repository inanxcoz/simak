...
function getFormSearch1()
{
	$CI = getCI();
	//include('form-search.php');
	$CI->load->view($CI->jCfg['theme'] . '/form-search1');
}
...
function link_action($links = array(), $id = "", $except = array())
{
	...
	?>
				<a href="<?php echo $m['link'] . "/" . $id; ?>" data-toggle="tooltip" <?php echo $property; ?> data-original-title="<?php echo ucwords($m['title']); ?>" title="<?php echo ucwords($m['title']); ?> "><span class="<?php echo $m['image']; ?>" style="font-size:21px;"></span></a>
	<?php
			}
		}
	}
...
function top_menu($m, $top = true)
{
	...
	if ($top) {
		echo '<li><a href="' . site_url('meme/me') . '" style="font-size:13px;solid;"><span class="fa fa-home"></span> Dashboard </a></li>';
	}
	...
	foreach ((array)$m as $k => $v) {
		if (is_array($v) && !isset($v['menu']) && !isset($v['id']) && !isset($v['name'])) {
			...
			echo '<a href="javascript:;" style="font-size:13px;"> <span class="' . $css_class . '"></span>' . $k . '</a>';
			top_menu($v, false);
			echo "</li>";
		} else {
			if (isset($v['name']) && trim($v['name']) !== "me" && trim($v['name']) !== "") {
				$css_class = isset($v[0]['css_class']) && trim($v[0]['css_class']) != "" ? $v[0]['css_class'] : ''; //'fa fa-folder-open';
				echo '<li>';
				echo '<a href="' . site_url($v['group'] . '/' . $v['name']) . '" style="font-size:13px;" ><span class="' . $css_class . '"></span> ' . $v['menu'] . '</a>';
				echo '</li>';
			}
		}
	}
...
function left_menu($m, $top = true)
{
	...
	echo ($top) ? '' : '<ul class="animated zoomIn">';
	if ($top) {
		echo '<li><a href="' . site_url('meme/me') . '"style="font-size:13px;"><span class="fa fa-home"></span><span class="xn-text">Dashboard</span></a></li>';
	}
	...
	foreach ((array)$m as $k => $v) {
		if (is_array($v) && !isset($v['menu']) && !isset($v['id']) && !isset($v['name'])) {
			...
			echo '<li class="xn-openable">';
			echo '<a href="javascript:;"style="font-size:13px;"> <span class="' . $css_class . '" style="font-size:16px;"></span><span class="xn-text">' . $k . '</span></a>';
			top_menu($v, false);
			echo "</li>";
		} else {
...
function getCity($id = "")
{
	...
	if (trim($id) != "") {
		$q = $CI->db->query("
			SELECT * FROM app_city WHERE city_id='" . $id . "'
			LIMIT 0,1
		")->row();
		if (count($q) > 0) {
			$city = $q->city_title;
...
function option_province($prov = "")
{
	...
	$q = $CI->db->get("app_province")->result();
...
function getProvince($id = "")
{
	...
	if (trim($id) != "") {
		$q = $CI->db->query("
			SELECT * FROM app_province WHERE province_id='" . $id . "'
			LIMIT 0,1
		")->row();
		if (count($q) > 0) {
			$province = $q->province_title;
		}
	}
...
{
	$CI  = getCI();
	$CI->db->order_by('sec_name', 'ASC');
	$CI->db->where('sec_status', 1);
	if ($CI->jCfg['user']['is_all'] != 1) {
		$CI->db->where("sec_id", $CI->jCfg['user']['section']);
	}
	if (trim($sec) != "") {
		$CI->db->where("sec_id", $sec);
	}
	return $CI->db->get("mdr_section")->result();
}
...
function option_section($dom = "")
{
	...
	if (count($q) > 0) {
		foreach ($q as $key => $value) {
			$selected = $value->sec_id == $dom ? "selected='selected'" : "";
			$callback .= "<option value='" . $value->sec_id . "' $selected >" . $value->sec_name . "</option>";
		}
	}
	return $callback;
}
